...
| **Tier-level approval enforcement** | The current `Approve_Discount_Request` custom permission is a single binary gate — any holder can approve any Pending request regardless of tier. `Required_Approver_Level__c` and queue routing are informational only. The fix is either separate permission sets per tier (`Discount_Approver_SalesManager`, `Discount_Approver_Director`, etc.) or a trigger-level check that validates the approving user is a member of the request's `OwnerId` queue before allowing the status change. |
