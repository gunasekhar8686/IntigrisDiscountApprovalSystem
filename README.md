...
- **No UI or inbox visibility:** Claude Code can run SOQL queries and retrieve Apex debug logs via the SF CLI — those were genuinely useful during debugging. What it cannot do is see what is rendered on screen in the browser, browse Setup pages visually, or access external inboxes. Two real examples from this project: emails sending successfully (confirmed via debug logs and `Messaging.sendEmail` returning no errors) but landing in Gmail spam required opening the inbox manually to diagnose. The Salesforce access token containing a `!` character breaking Postman's HTTP client was a client-side issue that only became visible when the actual error message appeared in the terminal — no amount of org introspection would have surfaced it.
